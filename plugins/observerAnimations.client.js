...
Vue.directive('animate', {
  ...
  inserted: (el, binding, vnode) => {
    ...
    const onExit = (instance) => {
      const classList = instance.target.classList
      const isAbove = instance.rootBounds.top > instance.boundingClientRect.top

      if (isAbove) {
        classList.add(el.classes.above)
        classList.remove(el.classes.below, el.classes.here)
      } else {
        classList.add(el.classes.below)
        classList.remove(el.classes.above, el.classes.here)
      }
    }
